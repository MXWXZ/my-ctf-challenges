...
MongoClient.connect(process.env.DB_URL, { useNewUrlParser: true, useUnifiedTopology: true }, function (e, client) {
    if (e) {
        ...
        exports.addNewAccount({
            _id: new ObjectID.createFromHexString("5f7fa2bbbfebf50020a41904"),
            name: 'admin',
            email: 'admin@0ops.sjtu.cn',
            user: 'admin',
            pass: '^gX91xQoLe!cVnvb',
            country: 'China'
        }, function (e) {
            if (!e)
...
exports.generatePasswordKey = function (email, ipAddress, callback) {
    ...
    switch (getRandomInt(3)) {
        case 0:
            mid = md5(ipAddress).substr(0, 6);
            break;
        case 1:
            mid = md5(ipAddress).substr(6, 6);
            break;
        case 2:
            mid = md5(ipAddress).substr(12, 6);
            break;
    }
    let passKey = md5((Math.trunc(((new Date().getTime()) / 1000))).toString()) + '-' + mid + getRandomInt(10);
