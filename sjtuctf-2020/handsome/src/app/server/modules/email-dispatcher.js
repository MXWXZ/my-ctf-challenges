...
EM.dispatchResetPasswordLink = function (account, callback) {
    ...
    if (account.email == 'admin@0ops.sjtu.cn')
        callback('Your reset url has been sent to your email.')
    else
        callback('Visit ' + baseurl + '/reset-password?key=' + account.passKey + ' to reset the password.')
    // EM.server.send({
