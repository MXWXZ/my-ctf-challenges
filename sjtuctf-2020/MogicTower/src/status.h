...
void setDefLoadFileName(){
	swprintf(fbuf,tbuflen,L"%s*.sav",dbuf);
}
...
void setDefSaveFileName(){
	...
	GetSystemTime(&time);
	swprintf(fbuf,tbuflen,L"%s%04d%02d%02d%02d%02d%02d.sav",dbuf,time.wYear,time.wMonth,time.wDay,time.wHour,time.wMinute,time.wSecond);
}
