...
LRESULT CALLBACK WndProc(HWND hwnd,UINT message,WPARAM wParam,LPARAM lParam){
	...
	case WM_CREATE:
		...
		target=CT_Null;
		swprintf(statusbuf,tbuflen,szStatus_Intro);
		dialogY=-1;
		story=ST_Intro;
		autosave=0;
	...
	case WM_APP:
		...
		case ST_Finale1:
			...
			rect.right=640;
			rect.bottom=112;
            szScreen_Finale1[6] = 100;
            szScreen_Finale1[0] = 95;
            szScreen_Finale2[2] = 122;
            szScreen_Finale1[1] = 83;
            szScreen_Finale2[0] = 95;
            szScreen_Finale1[4] = 118;
            szScreen_Finale2[3] = 49;
            szScreen_Finale1[3] = 111;
            szScreen_Finale3[1] = 80;
            szScreen_Finale1[2] = 104;
            szScreen_Finale1[5] = 49;
            DrawTextW(hdcMem, szScreen_Finale1, wcslen(szScreen_Finale1), &rect,
		...
		case ST_Finale2:
			...
			//rect.right=640;
			rect.bottom=176;
            szScreen_Finale2[4] = 89;
            szScreen_Finale3[3] = 83;
            szScreen_Finale2[1] = 101;
            szScreen_Finale3[2] = 97;
		...
		case ST_Finale3:
			...
			//rect.right=640;
			rect.bottom=240;
            szScreen_Finale3[0] = 95;
            szScreen_Finale3[4] = 83;
		...
		default:
			...
			rect.right=110;
			rect.bottom=34;
			swprintf(wbuf,tbuflen,szUI_Round,stmain.round);
			DrawTextW(hdcMem,wbuf,wcslen(wbuf),&rect,DT_NOPREFIX|DT_SINGLELINE|DT_CENTER|DT_VCENTER);
			//rect.left=18;
			rect.top=50;
			//rect.right=110;
			rect.bottom=66;
			swprintf(wbuf,tbuflen,L"%s %d%s",szUI_Tower,stmain.floor,szUI_Floor);
			DrawTextW(hdcMem,wbuf,wcslen(wbuf),&rect,DT_NOPREFIX|DT_SINGLELINE|DT_CENTER|DT_VCENTER);
			rect.left=38;
			rect.top=82;
			//rect.right=110;
			rect.bottom=98;
			swprintf(wbuf,tbuflen,L"%d",stmain.life);
			DrawTextW(hdcMem,wbuf,wcslen(wbuf),&rect,DT_NOPREFIX|DT_SINGLELINE|DT_RIGHT|DT_VCENTER);
			//rect.left=38;
			rect.top=114;
			//rect.right=110;
			rect.bottom=130;
			swprintf(wbuf,tbuflen,L"%d",stmain.attack);
			DrawTextW(hdcMem,wbuf,wcslen(wbuf),&rect,DT_NOPREFIX|DT_SINGLELINE|DT_RIGHT|DT_VCENTER);
			//rect.left=38;
			rect.top=146;
			//rect.right=110;
			rect.bottom=162;
			swprintf(wbuf,tbuflen,L"%d",stmain.defend);
			DrawTextW(hdcMem,wbuf,wcslen(wbuf),&rect,DT_NOPREFIX|DT_SINGLELINE|DT_RIGHT|DT_VCENTER);
			//rect.left=38;
			rect.top=178;
			//rect.right=110;
			rect.bottom=194;
			swprintf(wbuf,tbuflen,L"%d",stmain.gold);
			DrawTextW(hdcMem,wbuf,wcslen(wbuf),&rect,DT_NOPREFIX|DT_SINGLELINE|DT_RIGHT|DT_VCENTER);
			if (stmain.weapon>=0){
				rect.left=528;
				rect.top=32;
				rect.right=590;
				rect.bottom=48;
				swprintf(wbuf,tbuflen,L"%s",aszWeaponName[stmain.weapon]);
				DrawTextW(hdcMem,wbuf,wcslen(wbuf),&rect,DT_NOPREFIX|DT_SINGLELINE|DT_CENTER|DT_VCENTER);
			}
			if (stmain.armor>=0){
				rect.left=528;
				...
				rect.right=590;
				rect.bottom=96;
				swprintf(wbuf,tbuflen,L"%s",aszArmorName[stmain.armor]);
				DrawTextW(hdcMem,wbuf,wcslen(wbuf),&rect,DT_NOPREFIX|DT_SINGLELINE|DT_CENTER|DT_VCENTER);
			}
			rect.left=546;
			rect.top=119;
			rect.right=622;
			rect.bottom=135;
			swprintf(wbuf,tbuflen,L"%d",stmain.key[0]);
			DrawTextW(hdcMem,wbuf,wcslen(wbuf),&rect,DT_NOPREFIX|DT_SINGLELINE|DT_RIGHT|DT_VCENTER);
			//rect.left=546;
			rect.top=144;
			//rect.right=622;
			rect.bottom=160;
			swprintf(wbuf,tbuflen,L"%d",stmain.key[1]);
			DrawTextW(hdcMem,wbuf,wcslen(wbuf),&rect,DT_NOPREFIX|DT_SINGLELINE|DT_RIGHT|DT_VCENTER);
			//rect.left=546;
			rect.top=169;
			//rect.right=622;
			rect.bottom=185;
			swprintf(wbuf,tbuflen,L"%d",stmain.key[2]);
			DrawTextW(hdcMem,wbuf,wcslen(wbuf),&rect,DT_NOPREFIX|DT_SINGLELINE|DT_RIGHT|DT_VCENTER);
			if (target>CT_Null){
				rect.left=18;
				rect.top=258;
				rect.right=110;
				rect.bottom=274;
				swprintf(wbuf,tbuflen,L"%s",cell[target&CT_Mask].name);
				DrawTextW(hdcMem,wbuf,wcslen(wbuf),&rect,DT_NOPREFIX|DT_SINGLELINE|DT_CENTER|DT_VCENTER);
				if (target&CT_Detail){
					rect.left=38;
					rect.top=281;
					rect.right=110;
					rect.bottom=297;
					swprintf(wbuf,tbuflen,L"%d",cell[target&CT_Mask].life);
					DrawTextW(hdcMem,wbuf,wcslen(wbuf),&rect,DT_NOPREFIX|DT_SINGLELINE|DT_RIGHT|DT_VCENTER);
					rect.left=38;
					rect.top=304;
					rect.right=110;
					rect.bottom=320;
					swprintf(wbuf,tbuflen,L"%d",cell[target&CT_Mask].attack);
					DrawTextW(hdcMem,wbuf,wcslen(wbuf),&rect,DT_NOPREFIX|DT_SINGLELINE|DT_RIGHT|DT_VCENTER);
					rect.left=38;
					rect.top=327;
					rect.right=110;
					rect.bottom=343;
					swprintf(wbuf,tbuflen,L"%d",cell[target&CT_Mask].defend);
					DrawTextW(hdcMem,wbuf,wcslen(wbuf),&rect,DT_NOPREFIX|DT_SINGLELINE|DT_RIGHT|DT_VCENTER);
					rect.left=38;
					rect.top=350;
					rect.right=110;
					rect.bottom=366;
					swprintf(wbuf,tbuflen,L"%d",cell[target&CT_Mask].gold);
					DrawTextW(hdcMem,wbuf,wcslen(wbuf),&rect,DT_NOPREFIX|DT_SINGLELINE|DT_RIGHT|DT_VCENTER);
				}
				SetTextColor(hdcMem,RGB(0,0,0));
			...
			for (i=0;i<itemCount;++i){
				...
				if (stmain.item[i]>1){
					swprintf(wbuf,tbuflen,L"%d",stmain.item[i]);
				...
				for(j=0;j<3;++j){
					for (i=0;i<3;++i){
						...
						if (saveFloor[temp]<0||saveFloor[temp]==stsave[temp].floor){
							swprintf(wbuf,tbuflen,szScreen_LoadStatus,temp+1,stsave[temp].floor,stsave[temp].round,stsave[temp].life,stsave[temp].attack,stsave[temp].defend);
			...
			case ST_Book:
				...
				for (i=0;i<11;++i){
					...
					rect.right=5*32+88;
					rect.bottom=(i+2)*32;
					swprintf(wbuf,tbuflen,L"%d",cell[enemies[i]].life);
					DrawTextW(hdcMem,wbuf,wcslen(wbuf),&rect,DT_NOPREFIX|DT_SINGLELINE|DT_RIGHT|DT_VCENTER);
					rect.left=5*32+104;
					//rect.top=(i+1)*32+16;
					rect.right=5*32+176;
					//rect.bottom=(i+2)*32;
					swprintf(wbuf,tbuflen,L"%d",cell[enemies[i]].attack);
					DrawTextW(hdcMem,wbuf,wcslen(wbuf),&rect,DT_NOPREFIX|DT_SINGLELINE|DT_RIGHT|DT_VCENTER);
					rect.left=5*32+192;
					//rect.top=(i+1)*32+16;
					rect.right=5*32+264;
					//rect.bottom=(i+2)*32;
					swprintf(wbuf,tbuflen,L"%d",cell[enemies[i]].defend);
					DrawTextW(hdcMem,wbuf,wcslen(wbuf),&rect,DT_NOPREFIX|DT_SINGLELINE|DT_RIGHT|DT_VCENTER);
					rect.left=5*32+280;
					//rect.top=(i+1)*32+16;
					rect.right=5*32+352;
					//rect.bottom=(i+2)*32;
					swprintf(wbuf,tbuflen,L"%d",cell[enemies[i]].gold);
					DrawTextW(hdcMem,wbuf,wcslen(wbuf),&rect,DT_NOPREFIX|DT_SINGLELINE|DT_RIGHT|DT_VCENTER);
					SetTextColor(hdcMem,RGB(255,255,255));
					rect.left=5*32;
					rect.top=(i+1)*32;
					...
					if (temp<0){
						...
						swprintf(wbuf,tbuflen,L"%d",temp);
						DrawTextW(hdcMem,wbuf,wcslen(wbuf),&rect,DT_NOPREFIX|DT_SINGLELINE|DT_RIGHT|DT_VCENTER);
					}
				}
				break;
			case ST_Note:
				...
				swprintf(wbuf,tbuflen,L"%d/%d",getPageNo(),getPageLen());
				DrawTextW(hdcMem,wbuf,wcslen(wbuf),&rect,DT_NOPREFIX|DT_SINGLELINE|DT_CENTER|DT_VCENTER);
				//rect.left=4*32;
				rect.top=2*32+16;
				//rect.right=16*32;
				...
				if (temp){
					...
					//rect.right=16*32;
					rect.bottom=4*32;
					swprintf(wbuf,tbuflen,L"%d%s",notes[temp].floor,szUI_Floor);
					DrawTextW(hdcMem,wbuf,wcslen(wbuf),&rect,DT_NOPREFIX|DT_SINGLELINE|DT_CENTER|DT_VCENTER);
					//rect.left=4*32;
					rect.top=5*32;
					//rect.right=16*32;
	...
	case WM_COMMAND:
		...
		case MENU_New_Normal:
			if (MessageBoxW(hwnd,szQuestion_NewGameConfirm,szTitle,MB_ICONQUESTION|MB_YESNO|MB_DEFBUTTON2)==IDYES){
				...
				dialogY=-1;
				story=0;
				swprintf(statusbuf,tbuflen,szStatus_NewGame);
				SendMessage(hwnd,WM_APP,0,0);
			}
			return 0;
		case MENU_New_Easy:
		...
		case MENU_Load:
			...
			if (GetOpenFileNameW(&openfilename)){
				if (!(fp=_wfopen(fbuf,L"rb"))){
					swprintf(wbuf,tbuflen,szErr_OpenFile,fbuf);
				...
				dialogY=-1;
				story=0;
				swprintf(statusbuf,tbuflen,szStatus_LoadFile);
				SendMessage(hwnd,WM_APP,0,0);
			}
			return 0;
		case MENU_Save:
			...
			if (GetSaveFileNameW(&openfilename)){
				...
				fclose(fp);
				target=CT_Null;
				swprintf(statusbuf,tbuflen,szStatus_SaveFile);
				SendMessage(hwnd,WM_APP,0,0);
			}
			return 0;
		case MENU_Help:
	...
	case WM_KEYDOWN:
		...
		case 'R'://reset
			...
			dialogY=-1;
			story=ST_Null;
			swprintf(statusbuf,tbuflen,szStatus_Reset);
			SendMessage(hwnd,WM_APP,0,0);
			return 0;
		}
		...
		case VK_PRIOR://Page Up
			...
			if (!isNearStair()){
				swprintf(statusbuf,tbuflen,szStatus_OrbOfFlyingForbidden);
			}else if (stmain.floor<49&&stmain.floor<stmain.maxfloor){
				goUpStair();
				swprintf(statusbuf,tbuflen,szStatus_OrbOfFlyingUsed);
				enter(0);
			}else{
				swprintf(statusbuf,tbuflen,szStatus_OrbOfFlyingFailH);
			}
			break;
		case VK_NEXT://Page Down
			...
			if (!isNearStair()){
				...
				swprintf(statusbuf,tbuflen,szStatus_OrbOfFlyingFailL);
			}
			break;
		case 'S'://quick save
			target=CT_Null;
			if (stmain.item[IT_OrbOfHero]>0){
				st_save(SS_Quick);
				swprintf(statusbuf,tbuflen,szStatus_Save);
			}else{
				swprintf(statusbuf,tbuflen,szStatus_SaveForbidden);
			}
			SendMessage(hwnd,WM_APP,0,0);
			break;
		case 'H'://OrbOfHero
		...
		case 'N'://OrbOfWisdom
			...
			dialogY=6;
			swprintf(dialogbuf,tbuflen,notes[getNoteIndex()].dialog);
			story=ST_Note;
			SendMessage(hwnd,WM_APP,0,0);
			break;
		case 'W'://MagicElixir
			...
			stmain.life+=temp;
			swprintf(statusbuf,tbuflen,szStatus_MagicElixirUsed,temp);
			break;
		case 'P'://MagicMattock
			...
			swprintf(statusbuf,tbuflen,szStatus_MagicMattockUsed);
			if (temp){
				playSound(WAV_Open);
			}
			break;
		case 'B'://DestructibleBall
			...
			swprintf(statusbuf,tbuflen,szStatus_DestructibleBallUsed,temp);
			if (temp){
		...
		case 'J'://WarpStaff
			...
			if (stmain.map[stmain.floor][10-stmain.y][10-stmain.x]==CT_Null){
				...
				swprintf(statusbuf,tbuflen,szStatus_WarpStaffUsed);
			}else{
				swprintf(statusbuf,tbuflen,szStatus_WarpStaffFail);
			}
			break;
		case 'U'://WingsToFlyUp
			...
			if (stmain.floor<50&&stmain.map[stmain.floor+1][stmain.y][stmain.x]==CT_Null){
				...
				swprintf(statusbuf,tbuflen,szStatus_WingsToFlyUpUsed);
			}else{
				swprintf(statusbuf,tbuflen,szStatus_WingsToFlyUpFail);
			}
			break;
		case 'D'://WingsToFlyDown
			...
			if (stmain.floor>0&&stmain.map[stmain.floor-1][stmain.y][stmain.x]==CT_Null){
				...
				swprintf(statusbuf,tbuflen,szStatus_WingsToFlyDownUsed);
			}else{
				swprintf(statusbuf,tbuflen,szStatus_WingsToFlyDownFail);
			}
			break;
		case 'I'://SnowCrystal
			...
			swprintf(statusbuf,tbuflen,szStatus_SnowCrystalUsed);
			break;
		case 'K'://MagicKey
			...
			swprintf(statusbuf,tbuflen,szStatus_MagicKeyUsed);
			if (temp){
				playSound(WAV_Open);
			}
			break;
		case 'Q'://SuperMagicMattock
			...
			swprintf(statusbuf,tbuflen,szStatus_SuperMagicMattockUsed);
			if (temp){
				playSound(WAV_Open);
			}
			break;
...
int main(){
	...
	if (!hIcon){
		swprintf(wbuf,tbuflen,szErr_OpenFile,L"./res/TSW.ico");
	...
	errmsg=loadIcons();
	if (errmsg){
		swprintf(wbuf,tbuflen,szErr_OpenFile,errmsg);
		MessageBoxW(NULL,wbuf,argv[0],MB_ICONERROR);
		DestroyIcon(hIcon);
		return 0;
	}
