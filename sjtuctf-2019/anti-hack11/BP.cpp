...
void BP::GetData(const Vector<Data> v_data) { data = v_data; }
...
double BP::GetRand() { return 2.0 * rand() / RAND_MAX - 1.0; }
...
double BP::GetError(int cnt) {
    ...
    for (int i = 0; i < ou_num; i++)
        ans +=
            0.5 * (x[2][i] - data.at(cnt).y[i]) * (x[2][i] - data.at(cnt).y[i]);
    return ans;
}
...
double BP::GetAccu() {
    ...
    for (int i = 0; i < num; i++) {
        ...
        for (int j = 0; j < n; j++)
            ans +=
                0.5 * (x[2][j] - data.at(i).y[j]) * (x[2][j] - data.at(i).y[j]);
    }
    return ans / num;
}
...
void BP::CalcDelta(int cnt) {
    ...
    for (int i = 0; i < ou_num; i++)
        d[2][i] =
            (x[2][i] - data.at(cnt).y[i]) * x[2][i] * (A - x[2][i]) / (A * B);
    // 计算隐含层的delta值
    for (int i = 0; i < hd_num; i++) {
        double t = 0;
        for (int j = 0; j < ou_num; j++)
...
double BP::Sigmoid(const double x) { return A / (1 + exp(-x / B)); }
